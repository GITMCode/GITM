...
! Auroral heating
  Qaurora = 0.0
  Qaurora(1:nLons, 1:nLats, 1:nAlts) = &
    auroheat &
    *epsilon(1:nLons, 1:nLats, 1:nAlts)*( &
...
! Ion Precipitation heating
  ...
  QprecipIon = 0.0
  QprecipIon(1:nLons, 1:nLats, 1:nAlts) = &
    epsilon(1:nLons, 1:nLats, 1:nAlts)*( &
...
! 8. N2 vibration from Pavlov 1998a
  ...
  !!! 1500K <= Te <=6000K
  Av0 = (/2.025, -7.066, -8.211, -9.713, -10.353, -10.819, &
          -10.183, -12.698, -14.710, -17.538/)
  Bv0 = (/8.782e-4, 1.001e-2, 1.092e-2, 1.204e-2, 1.243e-2, &
          1.244e-2, 1.185e-2, 1.309e-2, 1.409e-2, 1.6e-2/)
  Cv0 = (/2.954e-7, -3.066e-6, -3.369e-6, -3.732e-6, -3.850e-6, &
          -3.771e-6, -3.570e-6, -3.952e-6, -4.249e-6, -4.916e-6/)
  Dv0 = (/-9.562e-11, 4.436e-10, 4.891e-10, 5.431e-10, 5.6e-10, &
          5.385e-10, 5.086e-10, 5.636e-10, 6.058e-10, 7.128e-10/)
  Fv0 = (/7.252e-15, -2.449e-14, -2.706e-14, -3.008e-14, &
          -3.1e-14, -2.936e-14, -2.769e-14, -3.071e-14, -3.3e-14, -3.941e-14/)
  ...
  !!! 1500K <= Te <=6000K
  Av1 = (/-3.413, -4.16, -5.193, -5.939, -8.261, &
          -8.185, -10.823, -11.273/)
  Bv1 = (/7.326e-3, 7.803e-3, 8.36e-3, 8.807e-3, 1.01e-2, &
          1.01e-2, 1.199e-2, 1.283e-2/)
  Cv1 = (/-2.2e-6, -2.352e-6, -2.526e-6, -2.669e-6, -3.039e-6, &
          -3.039e-6, -3.62e-6, -3.879e-6/)
  Dv1 = (/3.128e-10, 3.352e-10, 3.606e-10, 3.806e-10, 4.318e-10, &
          4.318e-10, 5.159e-10, 5.534e-10/)
  Fv1 = (/-1.702e-14, -1.828e-14, -1.968e-14, -2.073e-14, -2.347e-14, &
          -2.347e-14, -2.81e-14, -3.016e-14/)
...
!         else if (tte .LE. 6000 .AND. tte .GT. 1500) then
          ...
          logQv0(iLon, iLat, iAlt, :) = Av0 + Bv0*tte_6000 + Cv0*tte_6000**2 &
                                        + Dv0*tte_6000**3 + Fv0*tte_6000**4 - 16.
          logQv1(iLon, iLat, iAlt, :) = Av1 + Bv1*tte_6000 + Cv1*tte_6000**2 &
                                        + Dv1*tte_6000**3 + Fv1*tte_6000**4 - 16.
...
!!! Thermal Conduction Perpendicular to Magnetic Field Lines
  cos2dip = 1.-(B0(1:nLons, 1:nLats, 0:nAlts + 1, iUp_, iBlock) &
                /B0(1:nLons, 1:nLats, 0:nAlts + 1, iMag_, iBlock))**2
  magh2 = B0(1:nLons, 1:nLats, 0:nAlts + 1, iEast_, iBlock)**2 &
          + B0(1:nLons, 1:nLats, 0:nAlts + 1, iNorth_, iBlock)**2
  ...
  do iAlt = 0, nAlts + 1
    dtedphe = 0.0
    dtidphe = 0.0
    dtedphe(0:nLons + 1, 0:nLats + 1) = (te_con(1:nLons + 2, 0:nLats + 1, iAlt) &
                                         - te_con(-1:nLons, 0:nLats + 1, iAlt))/2./dLon
    dtidphe(0:nLons + 1, 0:nLats + 1) = (ti_con(1:nLons + 2, 0:nLats + 1, iAlt) &
                                         - ti_con(-1:nLons, 0:nLats + 1, iAlt))/2./dLon

    dtedtheta = 0.0
    dtidtheta = 0.0
    dtedtheta(0:nLons + 1, 0:nLats + 1) = (te_con(0:nLons + 1, 1:nLats + 2, iAlt) &
                                           - te_con(0:nLons + 1, -1:nLats, iAlt))/2./dLat
    dtidtheta(0:nLons + 1, 0:nLats + 1) = (ti_con(0:nLons + 1, 1:nLats + 2, iAlt) &
                                           - ti_con(0:nLons + 1, -1:nLats, iAlt))/2./dLat

    dledphe = 0.0
    dlidphe = 0.0
    dledphe(1:nLons, 1:nLats) = (lame(2:nLons + 1, 1:nLats, iAlt) &
                                 - lame(0:nLons - 1, 1:nLats, iAlt))/2./dLon
    dlidphe(1:nLons, 1:nLats) = (lami(2:nLons + 1, 1:nLats, iAlt) &
                                 - lami(0:nLons - 1, 1:nLats, iAlt))/2./dLon

    dledtheta = 0.0
    dlidtheta = 0.0
    dledtheta(1:nLons, 1:nLats) = (lame(1:nLons, 2:nLats + 1, iAlt) &
                                   - lame(1:nLons, 0:nLats - 1, iAlt))/2./dLat
    dlidtheta(1:nLons, 1:nLats) = (lami(1:nLons, 2:nLats + 1, iAlt) &
                                   - lami(1:nLons, 0:nLats - 1, iAlt))/2./dLat

    ! Qeconh(:,:,iAlt) = lame(:,:,iAlt) * cos2dip(:,:,iAlt) &
! / (6.37e6 + alts(:,:,iAlt))**2 * (cos2dec(:,:,iAlt) &
    !      * (te_con(1:nLons,2:nLats+1,iAlt) + te_con(1:nLons,0:nLats-1,iAlt) &
! - 2*te_con(1:nLons,1:nLats,iAlt)) / dLat**2 &
    !      + sin2dec(:,:,iAlt) / sin2theta  &
    !      * (te_con(2:nLons+1,1:nLats,iAlt) + te_con(0:nLons-1,1:nLats,iAlt)&
!  - 2*te_con(1:nLons,1:nLats,iAlt)) / dLon**2 &
    !      + 2*sindec(:,:,iAlt)*cosdec(:,:,iAlt)/sintheta &
! * (dtedphe(1:nLons,2:nLats+1) - dtedphe(1:nLons,0:nLats-1))/2./dLat &
    ...
    Qeconhm(1:nLons, 1:nLats, iAlt) = &
      lame(1:nLons, 1:nLats, iAlt)*cos2dip(1:nLons, 1:nLats, iAlt) &
      /(6.37e6 + alts(1:nLons, 1:nLats, iAlt))**2 &
      *(cos2dec(1:nLons, 1:nlats, iAlt) &
        ...
        *(dtedphe(1:nLons, 2:nLats + 1) - dtedphe(1:nLons, 0:nLats - 1))/2./dLat &
        - sindec(1:nLons, 1:nLats, iAlt)*cosdec(1:nLons, 1:nLats, iAlt)/sintheta**2 &
    ...
    Qeconhp(1:nLons, 1:nLats, iAlt) = 2.*lame(1:nLons, 1:nLats, iAlt)*cos2dip(1:nLons, 1:nLats, iAlt) &
                                      /(6.37e6 + alts(1:nLons, 1:nLats, iAlt))**2 &
                                      *(cos2dec(1:nLons, 1:nLats, iAlt)/dLat**2 &
                                        + sin2dec(1:nLons, 1:nLats, iAlt)/sin2theta/dLon**2)

    Qiconhm(1:nLons, 1:nLats, iAlt) = &
      lami(1:nLons, 1:nLats, iAlt)*cos2dip(1:nLons, 1:nLats, iAlt) &
      /(6.37e6 + alts(1:nLons, 1:nLats, iAlt))**2 &
      *(cos2dec(1:nLons, 1:nLats, iAlt) &
    ...
    !      * (ti_con(1:nLons,2:nLats+1,iAlt) + ti_con(1:nLons,0:nLats-1,iAlt) &
! - 2*ti_con(1:nLons,1:nLats,iAlt)) / dLat**2 &
    !      + sin2dec(1:nLons,1:nLats,iAlt) / sin2theta  &
...
!  Qiconhm = 0.
  ...
  eHeatingm = Qphe + Qencm + Qeicm + Qrotm + Qf + Qexc + Qvib_o2 + Qvib_n2 &
              + Qaurora + QprecipIon + Qenc_v + Qeic_v &
              + Qeconhm
