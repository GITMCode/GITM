...
contains
  ...
  subroutine netcdf_open_file(dir, cType, cTime, cL)
    ...
    integer :: dimid_lon, dimid_lat, dimid_alt, dimid_time, dimid_vars(3)
    integer :: varid_time
    ...
    if (nDims == 3) then
      ierr = nfmpi_def_dim(ncid, "z", nAlts_g, dimid_alt)
    endif
    ierr = nfmpi_def_dim(ncid, "time", 1_MPI_OFFSET_KIND, dimid_time)

    ! --- Global time attribute (ISO string, for human readability) ---
    ...
    ! Time (CF-compliant, enables xarray decode_times) ---
    write(coord_units, '("seconds since ",i4.4,"-01-01 00:00:00")') 1965
    ierr = nfmpi_def_var(ncid, "time", NF_DOUBLE, 1, [dimid_time], varid_time)
    attlen = int(len_trim(coord_units), MPI_OFFSET_KIND)
    ierr = nfmpi_put_att_text(ncid, varid_time, "units", attlen, trim(coord_units))
    attlen = 8_MPI_OFFSET_KIND
    ierr = nfmpi_put_att_text(ncid, varid_time, "calendar", attlen, "standard")
    ! longitude
    write(coord_units, '("degrees_east")')
    ierr = nfmpi_def_var(ncid, "lon", NF_DOUBLE, 1, [dimid_lon], coordids(1))
    attlen = int(len_trim(coord_units), MPI_OFFSET_KIND)
    ierr = nfmpi_put_att_text(ncid, coordids(1), "units", attlen, trim(coord_units))
    ! latitude
    write(coord_units, '("degrees_north")')
    ierr = nfmpi_def_var(ncid, "lat", NF_DOUBLE, 1, [dimid_lat], coordids(2))
    attlen = int(len_trim(coord_units), MPI_OFFSET_KIND)
    ierr = nfmpi_put_att_text(ncid, coordids(2), "units", attlen, trim(coord_units))
    ! Altitude
    if (nDims == 3) then
    ...
    ! Since all ranks hold the same CurrentTime), write happens here.
    ! Other coords are written later
    tstart(1) = 1_MPI_OFFSET_KIND
    tcnt(1) = 1_MPI_OFFSET_KIND
    time_val(1) = CurrentTime
    ierr = nfmpi_put_vara_double_all(ncid, varid_time, tstart, tcnt, time_val)
    if (ierr /= NF_NOERR) &
