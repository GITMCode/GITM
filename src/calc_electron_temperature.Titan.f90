...
! 8. N2 vibration from Pavlov 1998a
  ...
  Fv0 = (/7.252e-15, -2.449e-14, -2.706e-14, -3.008e-14, -3.1e-14, &
  ...
  Av1 = (/-3.413, -4.16, -5.193, -5.939, -8.261, &
          -8.185, -10.823, -11.273/)
  Bv1 = (/7.326e-3, 7.803e-3, 8.36e-3, 8.807e-3, &
          1.01e-2, 1.01e-2, 1.199e-2, 1.283e-2/)
  Cv1 = (/-2.2e-6, -2.352e-6, -2.526e-6, -2.669e-6, &
          -3.039e-6, -3.039e-6, -3.62e-6, -3.879e-6/)
  Dv1 = (/3.128e-10, 3.352e-10, 3.606e-10, 3.806e-10, &
          4.318e-10, 4.318e-10, 5.159e-10, 5.534e-10/)
  Fv1 = (/-1.702e-14, -1.828e-14, -1.968e-14, -2.073e-14, &
          -2.347e-14, -2.347e-14, -2.81e-14, -3.016e-14/)
    ...
    do iLat = 1, nLats
      do iAlt = 1, nAlts
        ...
        if (tte .LE. 1500 .AND. tte .GT. 300) then
          ...
          logQv0(iLon, iLat, iAlt, 1) = Av0L + Bv0L*tte + Cv0L*tte**2 &
                                        + Dv0L*tte**3 + Fv0L*tte**4 - 16.
          Qvib_n2(iLon, iLat, iAlt) = (1.-exp(-3353./ttn))* &
                                      10**logQv0(iLon, iLat, iAlt, 1) &
                                      *(1.-exp(1*3353.*(1./tte - 1./ttn)))
...
!       (nu_h2ch5p/nu_ii_ch5p)*(Din_h2ch5p + 1.5*(Ah2/(Ach5p + Ah2))) )
  ...
  lam_ch5p = 3.1e4*ti**2.5/Ach5p**0.5 &
             /(1 + 1.25* &
               ((nu_n2ch5p/nu_ii_ch5p)*(Din_n2ch5p + 1.5*(An2/(Ach5p + An2))) + &
                (nu_ch4ch5p/nu_ii_ch5p)*(Din_ch4ch5p + 1.5*(Ach4/(Ach5p + Ach4))) + &
                (nu_h2ch5p/nu_ii_ch5p)*(Din_h2ch5p + 1.5*(Ah2/(Ach5p + Ah2)))))

  lam_c2h5p = 3.1e4*ti**2.5/Ac2h5p**0.5 &
              /(1 + 1.25*( &
  ...
  lam_hcnhp = 3.1e4*ti**2.5/Ahcnhp**0.5 &
              /(1 + 1.25*( &
  ...
  do iAlt = 0, nAlts + 1
    ...
    Qeconhp(1:nLons, 1:nLats, iAlt) = &
      2.*lame(1:nLons, 1:nLats, iAlt)*cos2dip(1:nLons, 1:nLats, iAlt) &
...
!  eHeatingm  = Qphe + Qencm + Qeicm + Qrotm + Qf + Qexc + Qvib_o2 + &
! Qvib_n2 + Qaurora + QprecipIon + Qenc_v + Qeic_v &
!       + Qeconhm
